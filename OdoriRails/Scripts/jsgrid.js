...
(function(window, $, undefined) {
    ...
    Grid.prototype = {
        ...
        invalidNotify: function(args) {
            const messages = $.map(args.errors,
                function(error) {
        ...
        _initFields: function() {
            var self = this;
            self.fields = $.map(self.fields,
                function(field) {
                    if ($.isPlainObject(field)) {
                        const fieldConstructor = (field.type && jsGrid.fields[field.type]) || jsGrid.Field;
        ...
        _createLoadIndicator: function() {
            return getOrApply(this.loadIndicator,
                this,
                {
        ...
        _createHeader: function() {
            const $headerRow = this._headerRow = this._createHeaderRow();
            const $filterRow = this._filterRow = this._createFilterRow();
            const $insertRow = this._insertRow = this._createInsertRow();

            const $headerGrid = this._headerGrid = $('<table>').addClass(this.tableClass)
                .append($headerRow)
                .append($filterRow)
                .append($insertRow);

            const $header = this._header = $('<div>').addClass(this.gridHeaderClass)
        ...
        _createBody: function() {
            const $content = this._content = $('<tbody>');

            const $bodyGrid = this._bodyGrid = $('<table>').addClass(this.tableClass)
                .append($content);

            const $body = this._body = $('<div>').addClass(this.gridBodyClass)
                .append($bodyGrid)
                .on('scroll',
                    $.proxy(function(e) {
        ...
        _createPagerContainer: function() {
            const pagerContainer = this.pagerContainer || $('<div>').appendTo(this._container);
        ...
        _createHeaderRow: function() {
            ...
            this._eachField(function(field, index) {
                const $th = this._prepareCell('<th>', field, 'headercss', this.headerCellClass)
        ...
        _refreshContent: function() {
            ...
            const indexFrom = this._loadStrategy.firstDisplayIndex();
            const indexTo = this._loadStrategy.lastDisplayIndex();
        ...
        _createCell: function(item, field) {
            var $result;
            const fieldValue = this._getItemFieldValue(item, field);

            const args = { value: fieldValue, item: item };
        ...
        _setSortingCss: function() {
            const fieldIndex = this._visibleFieldIndex(this._sortField);
        ...
        _refreshPager: function() {
            const $pagerContainer = this._pagerContainer;
            ...
            const showPager = this.paging && this._pagesCount() > 1;
        ...
        _createPagerByFormat: function() {
            ...
            const pagerParts = this.pagerFormat.split(' ');
            return $.map(pagerParts,
                $.proxy(function(pagerPart) {
                        ...
                        if (pagerPart === PAGES_PLACEHOLDER) {
                            ...
                            result =
                                this._createPagerNavButton(this.pageNextText, pageIndex + 1, pageIndex < pageCount);
        ...
        _createPages: function() {
            const pageCount = this._pagesCount();
            const pageButtonCount = this.pageButtonCount;
            const firstDisplayingPage = this._firstDisplayingPage;
            const pages = [];
            ...
            for (var i = 0, pageNumber = firstDisplayingPage;
        ...
        _createPagerNavButton: function(text, pageIndex, isActive) {
            return this._createPagerButton(text,
                this.pagerNavButtonClass + (isActive ? '' : ` ${this.pagerNavButtonInactiveClass}`),
        ...
        _createPagerPage: function(pageIndex) {
            return this._createPagerButton(pageIndex,
                this.pageClass,
                function() {
        ...
        _createPagerButton: function(text, css, handler) {
            const $link = $('<a>').attr('href', EMPTY_HREF)
                .html(text)
        ...
        _refreshWidth: function() {
            const width = (this.width === 'auto') ? this._getAutoWidth() : this.width;

            this._container.width(width);
        },

        _getAutoWidth: function() {
            const $headerGrid = this._headerGrid;
            const $header = this._header;

            $headerGrid.width('auto');

            const contentWidth = $headerGrid.outerWidth();
            const borderWidth = $header.outerWidth() - $header.innerWidth();
        ...
        _scrollBarWidth: (function() {
            ...
            return function() {
                if (result === undefined) {
                    ...
                    const $ghostContent = $("<div style='height:100px;'></div>");
                    $ghostContainer.append($ghostContent).appendTo('body');
                    const width = $ghostContent.innerWidth();
                    $ghostContainer.css('overflow-y', 'auto');
                    const widthExcludingScrollBar = $ghostContent.innerWidth();
        ...
        showPrevPages: function() {
            ...
            this._firstDisplayingPage = (firstDisplayingPage > pageButtonCount)
        ...
        loadData: function(filter) {
            ...
            const args = this._callEventHandler(this.onDataLoading,
                {
                    filter: filter
                });

            return this._controllerCall('loadData',
                filter,
                args.cancel,
                function(loadedData) {
                    if (!loadedData)
        ...
        getSorting: function() {
            const sortingParams = this._sortingParams();
            return {
        ...
        insertItem: function(item) {
            ...
            const args = this._callEventHandler(this.onItemInserting,
                {
                    item: insertingItem
                });

            return this._controllerCall('insertItem',
                insertingItem,
                args.cancel,
        ...
        _getValidatedInsertItem: function() {
            const item = this._getInsertItem();
        ...
        _validateItem: function(item, $row) {
            ...
            this._eachField(function(field) {
                ...
                const errors = this._validation.validate($.extend({
                        value: fieldValue,
        ...
        clearInsert: function() {
            const insertRow = this._createInsertRow();
        ...
        _editRow: function($row) {
            ...
            const item = $row.data(JSGRID_ROW_DATA_KEY);

            const args = this._callEventHandler(this.onItemEditing,
                {
                    row: $row,
                    item: item,
            ...
            const $editRow = this._createEditRow(item);
        ...
        _createEditRow: function(item) {
            if ($.isFunction(this.editRowRenderer)) {
                return $(this.renderTemplate(this.editRowRenderer,
                    this,
                    { item: item, itemIndex: this._itemIndex(item) }));
            }
        ...
        updateItem: function(item, editedItem) {
            ...
            const $row = item ? this.rowByItem(item) : this._editingRow;
        ...
        _getValidatedEditedItem: function() {
            const item = this._getEditedItem();
        ...
        _updateRow: function($updatingRow, editedItem) {
            ...
            const args = this._callEventHandler(this.onItemUpdating,
                {
                    row: $updatingRow,
                    item: updatedItem,
            ...
            return this._controllerCall('updateItem',
                updatedItem,
                args.cancel,
                function(loadedUpdatedItem) {
                    const previousItem = $.extend(true, {}, updatingItem);
                    updatedItem = loadedUpdatedItem || $.extend(true, updatingItem, editedItem);

                    const $updatedRow = this._finishUpdate($updatingRow, updatedItem, updatingItemIndex);
        ...
        _finishUpdate: function($updatingRow, updatedItem, updatedItemIndex) {
            ...
            const $updatedRow = this._createRow(updatedItem, updatedItemIndex);
        ...
        _deleteRow: function($row) {
            ...
            const args = this._callEventHandler(this.onItemDeleting,
                {
                    row: $row,
                    item: deletingItem,
            ...
            return this._controllerCall('deleteItem',
                deletingItem,
                args.cancel,
                function() {
    ...
    $.fn.jsGrid = function(config) {
        ...
        this.each(function() {
            const $element = $(this);
            const instance = $element.data(JSGRID_DATA_KEY);
            var methodResult;
    ...
    const locale = function(lang) {
        const localeConfig = $.isPlainObject(lang) ? lang : locales[lang];
...
(function(jsGrid, $, undefined) {
    ...
    LoadIndicator.prototype = {
        ...
        show: function() {
            const $loadPanel = this._loadPanel.show();

            const actualWidth = $loadPanel.outerWidth();
            const actualHeight = $loadPanel.outerHeight();
...
(function(jsGrid, $, undefined) {
    ...
    DirectLoadingStrategy.prototype = {
        ...
        lastDisplayIndex: function() {
            const grid = this._grid;
            const itemsCount = grid.option('data').length;
...
(function(jsGrid, $, undefined) {
    ...
    Validation.prototype = {
        ...
        validate: function(args) {
            ...
            $.each(this._normalizeRules(args.rules),
                function(_, rule) {
...
(function(jsGrid, $, undefined) {
    ...
    TextField.prototype = new Field({
        ...
        filterTemplate: function() {
            ...
            var grid = this._grid;
            const $result = this.filterControl = this._createTextBox();
            if (this.autosearch) {
                $result.on('keypress',
                    function(e) {
                        if (e.which === 13) {
        ...
        editTemplate: function(value) {
            ...
            const $result = this.editControl = this._createTextBox();
            $result.val(value);
            return $result;
        },
...
(function(jsGrid, $, undefined) {
    ...
    TextAreaField.prototype = new TextField({
        ...
        editTemplate: function(value) {
            ...
            const $result = this.editControl = this._createTextArea();
            $result.val(value);
            return $result;
        },
...
(function(jsGrid, $, undefined) {
    ...
    function SelectField(config) {
        ...
        if (config.valueField && config.items.length) {
            const firstItemValue = config.items[0][config.valueField];
    ...
    SelectField.prototype = new NumberField({
        ...
        itemTemplate: function(value) {
            ...
            const result = (textField ? resultItem[textField] : resultItem);
        ...
        filterTemplate: function() {
            ...
            var grid = this._grid;
            const $result = this.filterControl = this._createSelect();
            if (this.autosearch) {
                $result.on('change',
                    function(e) {
                        grid.search();
        ...
        editTemplate: function(value) {
            ...
            const $result = this.editControl = this._createSelect();
            (value !== undefined) && $result.val(value);
            return $result;
        },

        filterValue: function() {
            const val = this.filterControl.val();
            return this.valueType === numberValueType ? parseInt(val || 0, 10) : val;
        },

        insertValue: function() {
            const val = this.insertControl.val();
            return this.valueType === numberValueType ? parseInt(val || 0, 10) : val;
        },

        editValue: function() {
            const val = this.editControl.val();
            return this.valueType === numberValueType ? parseInt(val || 0, 10) : val;
        },
        ...
        _createSelect: function() {
            ...
            $.each(this.items,
                function(index, item) {
                    const value = valueField ? item[valueField] : index;
...
(function(jsGrid, $, undefined) {
    ...
    CheckboxField.prototype = new Field({
        ...
        filterTemplate: function() {
            ...
            var grid = this._grid;
            const $result = this.filterControl = this._createCheckbox();
            $result.prop({
                readOnly: true,
                indeterminate: true
            });
        ...
        editTemplate: function(value) {
            ...
            const $result = this.editControl = this._createCheckbox();
...
(function(jsGrid, $, undefined) {
    ...
    ControlField.prototype = new Field({
        ...
        headerTemplate: function() {
            ...
            const hasFiltering = this._hasFiltering;
            const hasInserting = this._hasInserting;
        ...
        filterTemplate: function() {
            const $result = this._createSearchButton();
        ...
        _createOnOffSwitchButton: function(option, cssClass, isOnInitially) {
            ...
            var $button = this._createGridButton(this.modeButtonClass + ' ' + cssClass,
                '',
                function(grid) {
                    isOn = !isOn;
        ...
        _createModeSwitchButton: function() {
            ...
            var $button = this._createGridButton(this.modeButtonClass,
                '',
                function(grid) {
        ...
        _createSearchButton: function() {
            return this._createGridButton(this.searchButtonClass,
        ...
        _createClearFilterButton: function() {
            return this._createGridButton(this.clearFilterButtonClass,
        ...
        _createInsertButton: function() {
            return this._createGridButton(this.insertButtonClass,
        ...
        _createUpdateButton: function() {
            return this._createGridButton(this.updateButtonClass,
        ...
        _createCancelEditButton: function() {
            return this._createGridButton(this.cancelEditButtonClass,
