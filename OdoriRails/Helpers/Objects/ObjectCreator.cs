...
namespace OdoriRails.Helpers.Objects
{
    public class ObjectCreator
    {
        ...
        public Sector CreateSector(DataRow row)
        {
            ...
            return new Sector((int) array[0], (int) array[2], (SectorStatus) array[1], occupyingTramNumber, latitude,
                longitude);
        }
        ...
        public Tram CreateTram(DataRow row)
        {
            ...
            var driver = row["DriverFk"] == DBNull.Value
                ? null
                : CreateUser(_userContext.GetUser((int) row["DriverFk"]));
        ...
        public Repair CreateRepair(DataRow row)
        {
            ...
            var users = GenerateListWithFunction(_serviceContext.GetUsersInServiceById((int) row["ServiceFk"]),
                CreateUser);
